...
import hegemonpack.HegemonPack;
import hegemonpack.util.TriFunction;
...
import static hegemonpack.util.GeneralUtils.removePrefix;
import static hegemonpack.util.TextureLoader.getCardTextureString;
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    HegemonPack.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
